<%@ page language="java" contentType="text/html; charset=UTF-8"
   pageEncoding="UTF-8"%>
<%@ page import="employee.model.vo.Employee,board.model.vo.Board,java.util.ArrayList"%>
<%
...
function showDiv(){
	if($("option[id=item]").eq(0).is(":checked")){
		$("#titlediv").css("display", "block");
		$("#writerdiv").css("display", "none");
	}	
	if($("option[id=item]").eq(1).is(":checked")){
...
function callFunction(){
	//자바스크립트로 서블릿 요청할 경우
	window.location.href = "/nado/views/groupware/board/boardWriteForm.jsp";
...
<div style="align:center;width:1200px;height:400px;">
<table align="center" width="1100" border="0" cellspacing="0" cellpadding="5">
<tr><th colspan="6">
<h6 align="right">조회갯수 : <%= listCount %> 개</h6><h6 align="right"><a href="/nado/blist">전체 목록 보기</a></h6><br>
</th></tr>
<tr>
		<th>번호</th>
...
<tr><th colspan="6"><hr size="5" width="100%"color="black" noshade align="center"></th></tr>

<% for(Board b : list){ %>
<tr>
		<td align="center"><strong><%= b.getBoardNo() %></strong></td>
		<td><a href="/nado/bdetail?no=<%= b.getBoardNo() %>"><%= b.getBoardTitle() %></a></td>
		<td><%= b.getUserId() %></td>
		<td align="center">
				<% if(b.getbFileOriname() != null && !b.getbFileOriname().equals("null")){ %>
						◎
				<% }else{ %>
						&nbsp;
				<% } %>
...
<a href="/nado/bsearch?page=1&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>">|◁</a> &nbsp;
<% if((beginPage - 10) < 1){ %>
	<a href="/nado/bsearch?page=1&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>">◀◀</a>
<% }else{ %>
	<a href="/nado/bsearch?page=<%= beginPage - 10 %>&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>">◀◀</a>
<% } %> &nbsp;
<% for(int p = beginPage; p <= endPage; p++){ 
		if(p == currentPage){
%>
	<a href="/nado/bsearch?page=<%= p %>&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>"><font color="red"><b>[<%= p %>]</b></font></a>
<% }else{ %>
	<a href="/nado/bsearch?page=<%= p %>&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>"><%= p %></a>
<% }}  %> &nbsp;
<% if((endPage + 10) > maxPage){ %>
	<a href="/nado/bsearch?page=<%= maxPage %>&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>">▶▶</a>
<% }else{ %>
	<a href="/nado/bsearch?page=<%= endPage + 10 %>&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>">▶▶</a>
<% } %> &nbsp;
<a href="/nado/bsearch?page=<%= maxPage %>&Titlekeyword=<%= titlekeyword%>&Writerkeyword=<%= writerkeyword%>">▷|</a>


<div class="searchbox" style="text-align: center; width: 280px;">
		<select name="item" style="float: left">
		<option id="item" value="title"> 제목</option>
		<option id="item" value="writer"> 아이디</option>
		</select>
		
		<div id="titlediv" style="float: right;">
		<form action="/nado/bsearch" method="get">
			<input type="search" name="Titlekeyword" placeholder="제목으로 검색">
			<input type="submit" value="검색">
		</form>
		</div>
		
		<div id="writerdiv" style="float: right;">
		<form action="/nado/bsearch" method="get">
			<input type="search" name="Writerkeyword" placeholder="아이디로 검색">
			<input type="submit" value="검색">
		</form>
		</div>
		
		
		<% if(manager.equals("M")){ %><div style="float: right;">
		<button onclick="callFunction();">새 게시글 등록</button>
		</div>
		<% }%>
