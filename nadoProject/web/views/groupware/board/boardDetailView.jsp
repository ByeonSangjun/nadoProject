<%@ page language="java" contentType="text/html; charset=UTF-8"
   pageEncoding="UTF-8"%>
<%@ page import="employee.model.vo.Employee,board.model.vo.Board,board.model.vo.Comments,java.util.ArrayList"%>
<%
...
function commentsreply(){	//답글달기 버튼 누르면 (수정, 삭제, 답글달기) 버튼사라지고 대댓글입력창 생성
	$(".commentsreply").css("display","block");
	$(".reply").css("display","none");
	$(".su").css("display","none");
	$(".sak").css("display","none");
}
function su(){	//수정버튼 누르면 (수정, 삭제)버튼사라지고 (저장, 취소)버튼생성
	$(".su").css("display","none");
	$(".sak").css("display","none");
	$(".qu").css("display","block");
	$(".ok").css("display","block");	
}
function qu(){	//취소버튼 누르면 (수정, 삭제)버튼생성되고 (저장, 취소)버튼사라짐
	$(".su").css("display","block");
	$(".sak").css("display","block");
	$(".qu").css("display","none");
	$(".ok").css("display","none");
}
function back(){	//답글달기에서 취소버튼 누르면 (수정, 삭제, 답글달기) 버튼나타나고 대댓글입력창 사라짐
...
<% if(loginEmp.getUserId().equals(board.getUserId())){ %><!-- 본인이 쓴 글일 때 -->

 <!-- 상세조회 부분 시작 -->
 <div style="align:center;width:100%;">
<form action="/nado/bupdate" method="post" enctype="multipart/form-data">
<input type="hidden" name="boardno" value="<%= board.getBoardNo() %>">
<input type="hidden" name="ofile" value="<%= board.getbFileOriname() %>">
<input type="hidden" name="rfile" value="<%= board.getbFileRename() %>">

<table align="center" style="border:0; width:1100px; cellspacing:0; cellpadding:0;">
<tr><th colspan="4"><h6 align="right"><a href="/nado/blist">전체 목록 보기</a></h6></th></tr>
<tr><th colspan="4" style="text-align:right; padding-bottom: 10px;">
			...
			<a href="/nado/bdel?no=<%= board.getBoardNo() %>&rfile=<%= board.getbFileRename() %>">삭제하기</a> &nbsp;
			<a href="JavaScript:window.location=document.referrer;">이전 페이지로</a>
			</th></tr>
<tr><th width="60%" style="padding-left:20px;">[<%= board.getBoardNo() %>]&nbsp;&nbsp;&nbsp;&nbsp;
<input type="text" name="title" class="viewww" value="<%= board.getBoardTitle() %>" style="width:500px;"></th>
<th width="18%">작성자 :&nbsp;<input type="text" name="writer" class="viewww" value="<%= board.getUserId() %>" disabled style="background:white;width:120px;"></th>
<th width="70px"><input type="date" name="date" class="viewww" value="<%= board.getBoardDate() %>" disabled style="background:white;width:130px;" ></th>
<th width="70px">조회수	&nbsp;<%= board.getBoardReadCount() %></th></tr>
...
<tr><th colspan="4"><textarea name="ir1" id="ir1" rows="15" cols="148"><%= board.getBoardContent() %></textarea></th></tr>
...
<tr>
		...
		<% if(board.getbFileOriname() != null && !board.getbFileOriname().equals("null")){ %>
		<a href="/nado/bfdown?ofile=<%= board.getbFileOriname() %>&rfile=<%= board.getbFileRename() %>">
		<%= board.getbFileOriname() %></a>&nbsp;
		<input type="checkbox" name="checkfile" value="<%= board.getbFileOriname() %>">삭제하기
		<% }else{ %>
			첨부파일 없음
		<% } %> <br>
...
<tr><th width="60%" style="padding-left:20px;">[<%= board.getBoardNo() %>]&nbsp;&nbsp;&nbsp;&nbsp;
<input type="text" class="viewww" value="<%= board.getBoardTitle() %>" disabled style="background:white;width:500px;"></th>
<th width="18%">작성자 :&nbsp;<input type="text" class="viewww" value="<%= board.getUserId() %>" disabled style="background:white;width:120px;"></th>
<th width="70px"><input type="date" name="date" class="viewww" value="<%= board.getBoardDate() %>" disabled style="background:white;width:130px;"></th>
<th width="70px">조회수	&nbsp;<%= board.getBoardReadCount() %></th></tr>
<tr><th colspan="4"><hr style="border:1px solid lightgray; margin-top:10px; margin-bottom:10px" width="100%"noshade align="center"></th></tr>
<tr><th colspan="4" style="padding-left:20px;"><div id="ir1content" style="overflow: auto; width:100%;"><%= board.getBoardContent() %></div></th></tr>
<tr><th colspan="4"><hr style="border:1px solid lightgray; margin-top:0px; margin-bottom:10px" width="100%"noshade align="center"></th></tr>
<tr>
		 <th colspan="4" style="text-align:left; padding-left:40px; padding-bottom:10px;">
		<% if(board.getbFileOriname() != null && !board.getbFileOriname().equals("null")){ %>
...
<!-- 댓글 테이블 시작 -->
<table align="center" style="border:0; width:1100px; cellspacing:0; cellpadding:0;">
<tr><th colspan="4" style="padding-left:20px; color:gray; background-color:lightgray;"><h5><%= list.size() %> 개의 댓글</h5></th></tr>
<% for(Comments c : list){ %>
<tr><th colspan="4" style="padding-left:50px; color:gray; background-color:lightgray;">
<div style="width:80%; background-color:lightgray; display:inline">
<form action="/nado/breplyupdate">
<input type="hidden" name="cmno" value="<%= c.getCmNo() %>">
<%if(c.getCmReplyLev() == 1 && (loginEmp.getUserId().equals(c.getUserId()))){ %><!-- 본인이 쓴 댓글 일 때 -->
<%=c.getUserId() %>&nbsp;&nbsp;&nbsp;<%= c.getCmDate() %><br>
<h4 style="display:inline;"><textarea cols="90" class="cmcomtent" name="cmcomtent" disabled="disabled"><%= c.getCmContent() %></textarea></h4>
<input type="button" value="답글달기" style="float:right; margin-right:30px; display:inline;" class="reply" onclick="commentsreply(); return false;">
<input type="button" value="삭제" style="float:right; margin-right:30px; display:inline;" class="sak" onclick="location.href='/nado/breplydel?cmno=<%= c.getCmNo() %>&cmreplylev=<%= c.getCmReplyLev() %>'">
<input type="button" value="수정" style="float:right; margin-right:30px; display:inline;" class="su" onClick="su(); this.form.cmcomtent.disabled=false; return false;" > <!-- 활성 -->
<input type="button" value="취소" style="float:right; margin-right:30px; display:none;"class="qu" onclick="qu(); this.form.cmcomtent.disabled=true; return false;"> <!-- 비활성 -->
<input type="submit" value="저장" style="float:right; margin-right:30px; display:none;"class="ok">

<%}else if(c.getCmReplyLev() == 1 && (!(loginEmp.getUserId().equals(c.getUserId())))){ %><!-- 본인이 쓴 댓글이 아닐 때 -->
<%=c.getUserId() %>&nbsp;&nbsp;&nbsp;<%= c.getCmDate() %><br>
<h4 style="display:inline;"><textarea cols="90" class="cmcomtent" name="cmcomtent" disabled="disabled"><%= c.getCmContent() %></textarea></h4>
<input type="button" value="답글달기" style="float:right; margin-right:30px; display:inline;" class="reply" onclick="commentsreply(); return false;">
<input type="button" value="취소" style="float:right; margin-right:30px; display:none;"class="qu" onclick="qu(); this.form.cmcomtent.disabled=true; return false;"> <!-- 비활성 -->
...
<%}else if(c.getCmReplyLev() == 2 && (loginEmp.getUserId().equals(c.getUserId()))){ %><!-- 본인이 쓴 대댓글 일 때 -->
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└>&nbsp;<%=c.getUserId() %>&nbsp;&nbsp;&nbsp;<%= c.getCmDate() %><br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<h4 style="display:inline;"><textarea cols="90" class="cmcomtent" name="cmcomtent" disabled="disabled"><%= c.getCmContent() %></textarea></h4>
<input type="button" value="삭제" style="float:right; margin-right:30px; display:inline;" class="sak" onclick="location.href='/nado/breplydel?cmno=<%= c.getCmNo() %>&cmreplylev=<%= c.getCmReplyLev() %>'">
<input type="button" value="수정" style="float:right; margin-right:30px; display:inline;" class="su" onClick="su(); this.form.cmcomtent.disabled=false; return false;" > <!-- 활성 -->
...
<%}else if(c.getCmReplyLev() == 2 && (!(loginEmp.getUserId().equals(c.getUserId())))){ %><!-- 본인이 쓴 대댓글이 아닐 때 -->
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;└>&nbsp;<%=c.getUserId() %>&nbsp;&nbsp;&nbsp;<%= c.getCmDate() %><br>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<h4 style="display:inline;"><textarea cols="90" class="cmcomtent" name="cmcomtent" disabled="disabled"><%= c.getCmContent() %></textarea></h4>
...
<!-- 대댓글 입력창 시작 -->
	<form action="/nado/breplyinsert2">
	<div class="commentsreply" style="width:80%; background-color:lightgray; display:none;">
	<input type="hidden" name="userid" value="<%= loginEmp.getUserId() %>">
	<input type="hidden" name="boardno" value="<%= board.getBoardNo() %>">
	<input type="hidden" name="cmreplylev" value="<%= c.getCmReplyRef() %>">
	<h4 style="display:inline;"><textarea name="cmcontent" placeholder="  답글을 입력하세요." cols="90"></textarea></h4>
	<input type="submit" value="등록" style="float:right; margin-right:10px; display:inline;">
	<input type="button" value="취소" style="float:right; margin-right:10px; display:inline;" class="replyqu" onclick="back(); return false;">
	</div>
	</form>
	<hr style="align:left; margin-right:20px; color:gray;">	
<!-- 대댓글 입력창 끝 -->
...
<!-- 댓글 입력창 시작 -->
<tr><th colspan="4" style="padding-left:50px; padding-bottom:10px; padding-top:15px; padding-right:50px; background-color:lightgray;">
<form action="/nado/breplyinsert" method="post">
<textarea style="width:100%; height:130px;"placeholder="  댓글을 입력하세요." name="replycontent"></textarea>
<input type="hidden" name="replyuser" value="<%= loginEmp.getUserId() %>">
<input type="hidden" name="boardno" value="<%= board.getBoardNo() %>">
<br><input type="submit" value="등록" style="float:right">
</form>
</th></tr>
<!-- 댓글 입력창 끝 -->
