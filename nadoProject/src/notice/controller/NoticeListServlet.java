...
public class NoticeListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int listCount = nservice.getListCount(session.getAttribute("comp"));  //테이블의 전체 목록 갯수 조회
		//총 페이지 수 계산
		...
		// 관리자용 공지사항 관리 페이지 컨트롤러
		
		ArrayList<Notice> list = nservice.selectList(session.getAttribute("comp"), startRow, endRow);
		
		RequestDispatcher view = null;
		if(list.size() > 0) {
			view = request.getRequestDispatcher("views/groupware/notice/noticeListView.jsp");
