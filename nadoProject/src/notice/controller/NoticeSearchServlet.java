...
/**
 * Servlet implementation class NoticeAdminSearchServlet
 */
...
public class NoticeSearchServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 게시글 전체 목록 조회 처리용 컨트롤러 : 페이징 처리됨
				...
				int listCount = nservice.getSearchListCount(session.getAttribute("comp"), titlekeyword, writerkeyword);  //테이블의 검색한 전체 목록 갯수 조회
				...
				// 관리자용 공지글 검색 처리용 컨트롤러
				// 제목, 작성자 검색 기능
				
				ArrayList<Notice> list = nservice.selectSearchList(session.getAttribute("comp"), startRow, endRow, titlekeyword, writerkeyword);
			
				RequestDispatcher view = null;
				if(list.size() > 0) {
					view = request.getRequestDispatcher("views/groupware/notice/noticeSearchListView.jsp");
